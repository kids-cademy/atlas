...
com.kidscademy.sync.App = function() {
	this.$super();
...
com.kidscademy.sync.App.prototype = {
	...
	_onItemRevealEvent : function(event) {
		...
		default:
			$error("com.kidscademy.sync.App#_onItemRevealEvent", "Not handled item type |%s|.", data.type);
			return;
		}
	...
	_onDisconnectEvent : function(event) {
		$debug("com.kidscademy.sync.App#_onDisconnectEvent", "Close server sent events stream.");
		this._events.close();
	...
	_onKeepAliveEvent : function(event) {
		$debug("com.kidscademy.sync.App#_onKeepAliveEvent", "Keep alive event.");
	},
	...
	toString : function() {
		return "com.kidscademy.sync.App";
	}
};
$extends(com.kidscademy.sync.App, js.ua.Page);
